...
// Espressif patches sets '_global_impure_ptr' to NULL, but it is used by '__register_exitproc()' which is called from `_start`.
// So we either need to use custom version of crt0.o for QEMU to avoid calling `__register_exitproc()` or implement dummy reent struct here 
// and initialize `_global_impure_ptr` by calling `__initreent()` from `_start` in crt1-sim.o as we do for Xtensa.
// At this momemtn the second option is easier to implement.
static struct _reent __ATTRIBUTE_IMPURE_DATA__ impure_data = _REENT_INIT (impure_data);


void __initreent(void)
{
    _GLOBAL_REENT = &impure_data;
}
